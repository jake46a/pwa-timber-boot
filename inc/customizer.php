<?php
/**
 * PWA - Timber-Boot Customizer
 * @package PWA_Starter_Site
 */
...
function pwa_customize_register( $wp_customize ) {
...
$wp_customize->add_control( new WP_Customize_Control($wp_customize, 'preset_color_scheme_setting', array(
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'header_bg_color',
            array(
                'label'      => __( 'Header Banner Background Color', 'pwa' ),
    ...
    $wp_customize->add_setting( 'header_banner_tagline_setting', array(
        'default' => __( 'To customize the contents of this header banner and other elements of your site go to Dashboard - Appearance - Customize','pwa' ),
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_tagline_setting', array(
        'label' => __( 'Banner Tagline', 'pwa' ),
        'section'    => 'header_image',
        'settings'   => 'header_banner_tagline_setting',
        'type' => 'text'
    ) ) );

    $wp_customize->add_setting( 'header_banner_tagbutton_setting', array(
        'default' => __( 'Tell Me More','pwa' ),
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_tagbutton_setting', array(
        'label' => __( 'Button Label', 'pwa' ),
        'section'    => 'header_image',
        'settings'   => 'header_banner_tagbutton_setting',
        'type' => 'text'
    ) ) );

    $wp_customize->add_setting( 'header_banner_buturl_setting', array(
        'default' => __( '/home','pwa' ),
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );

    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_buturl_setting', array(
        'label' => __( 'Button URL', 'pwa' ),
        'section'    => 'header_image',
        'settings'   => 'header_banner_buturl_setting',
        'type' => 'text'
    ) ) );
    ...
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_visibility', array(
        ...
        'label'    => __('Remove Header Banner', 'pwa'),
        'section'    => 'header_image',
        'type'     => 'checkbox',
    ) ) );
   ...
   $wp_customize->add_section(
        ...
        array(
            'title' => __( 'Other Customizations', 'pwa' ),
            //'description' => __( 'This is a section for the header banner Image.', 'pwa' ),
            'priority' => 40,
        )
    );
    $wp_customize->add_section(
    ...
    $wp_customize->add_setting( 'pwa_logo', array(
        //'default' => __( '', 'pwa' ),
        'sanitize_callback' => 'esc_url',
    ) );
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'pwa_logo', array(
        'label'    => __( 'Upload Logo (replaces text)', 'pwa' ),
        'section'  => 'title_tagline',
        'settings' => 'pwa_logo',
    ) ) );

}
add_action( 'customize_register', 'pwa_customize_register' );

add_action( 'wp_head', 'pwa_customizer_css');
function pwa_customizer_css()
{
    ?>
    <style type="text/css">
...
function pwa_customize_preview_js() {
    wp_enqueue_script( 'pwa_customizer', get_template_directory_uri() . '/inc/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'pwa_customize_preview_js' );
